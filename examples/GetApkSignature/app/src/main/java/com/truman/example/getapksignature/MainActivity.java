...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mTvStatus.setTextIsSelectable(true);
        mPackageHelper = new PackageHelper(getApplicationContext());
    }

    private void run() {
        Log.d(TAG, "run() - Inside");
        Signature[] signatures = mPackageHelper.getApkSignatures();
        if (signatures == null) {
            updateTv("Failed to get APK signatures\n");
            return;
        }

        updateTv("Found " + signatures.length + " signatures!\n");

        for (int i = 0 ; i < signatures.length; i++) {
            Signature signature = signatures[i];
            String sigInfo = String.format("Raw Signature[%d][Len : %d] :\n%s\n",
                    i,
